...
PKG_SRC = ROOT / "pkgs" / "tigrbl-auth-api-resource-validation" / "src"
if str(PKG_SRC) not in sys.path:
    sys.path.insert(0, str(PKG_SRC))

resource_validation_package = import_module("tigrbl_auth_api_resource_validation")
PRODUCT_SURFACE = resource_validation_package.PRODUCT_SURFACE
RESOURCE_VALIDATION_API_CONTRACT = (
    resource_validation_package.RESOURCE_VALIDATION_API_CONTRACT
)
...
def test_resource_validation_contract_routes_are_validation_only() -> None:
    ...
    for capability in RESOURCE_VALIDATION_API_CONTRACT.production_capabilities:
    ...
    for prefix in RESOURCE_VALIDATION_API_CONTRACT.forbidden_route_prefixes:
        assert all(
