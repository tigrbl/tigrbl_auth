...
PKG_SRC = ROOT / "pkgs" / "tigrbl-auth-backend-app-platform-admin" / "src"
if str(PKG_SRC) not in sys.path:
    sys.path.insert(0, str(PKG_SRC))

platform_admin_package = import_module("tigrbl_auth_backend_app_platform_admin")
PRODUCT_SURFACE = platform_admin_package.PRODUCT_SURFACE
PLATFORM_ADMIN_BACKEND_APP_CONTRACT = platform_admin_package.PLATFORM_ADMIN_BACKEND_APP_CONTRACT
...
def test_platform_admin_contract_matches_product_surface_registry() -> None:
    ...
    assert PRODUCT_SURFACE == "platform-admin-app"
    assert PLATFORM_ADMIN_BACKEND_APP_CONTRACT.product_surface == PRODUCT_SURFACE
    assert PLATFORM_ADMIN_BACKEND_APP_CONTRACT.intended_uix == (
    ...
    assert tuple(deployment.allowed_admin_resources) == (
        PLATFORM_ADMIN_BACKEND_APP_CONTRACT.admin_resources
    )
    assert tuple(deployment.allowed_admin_rest_groups) == (
        PLATFORM_ADMIN_BACKEND_APP_CONTRACT.admin_rest_groups
    )
...
def test_platform_admin_contract_routes_are_platform_control_plane_only() -> None:
    ...
    for route in PLATFORM_ADMIN_BACKEND_APP_CONTRACT.forbidden_exact_routes:
