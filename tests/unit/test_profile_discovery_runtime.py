...
def test_hardening_openapi_contract_reflects_runtime_constraints():
    ...
    assert any(
        param["name"] == "request_uri" and param["required"]
        for param in authorize_get["parameters"]
    )
    policy = runtime_security_profile(resolve_deployment(None, profile="hardening"))
    assert token_post["requestBody"]["content"]["application/x-www-form-urlencoded"][
        "schema"
    ]["properties"]["grant_type"]["enum"] == list(policy.allowed_grant_types)
    assert (
        "password"
        not in token_post["requestBody"]["content"][
