...
def test_authority_graph_robustness_t2_cycles_terminate_and_inactive_edges_are_excluded() -> (
    None
):
    graph = AuthorityDerivationGraph(
        ...
        edges=(
            ...
            AuthorityEdge(
                ...
                "authority:client",
                "grant",
                (AuthorityScope("tenant-a", "client.delete"),),
                active=False,
            ),
        ),
    )
...
def test_authority_graph_robustness_t2_detects_conflicting_versions_and_cross_tenant_edges() -> (
    None
):
    graph = AuthorityDerivationGraph(
        nodes=(
        ...
        edges=(
            ...
            AuthorityEdge(
                ...
                "role:admin",
                "grant",
                (AuthorityScope("tenant-b", "client.read"),),
                policy_version="v2",
            ),
        ),
    )
    ...
    assert (
        "conflicting policy versions for edge 'subject:alice'->'role:admin'"
        in report.failures
    )
    assert (
        "edge 'edge:tenant-b' scope crosses source tenant 'tenant-a'" in report.failures
    )


def test_authority_graph_robustness_t2_multi_hop_attenuation_prevents_downstream_escalation() -> (
    None
):
    graph = AuthorityDerivationGraph(
        nodes=_nodes(),
    ...
    proof = graph.prove_reachability(
        "subject:alice", AuthorityScope("tenant-a", "client.write")
    )

    assert proof.reachable is False
    assert tuple(
        scope.action
        for scope in compute_authority_closure(graph, "subject:alice").scopes
    ) == ("client.read",)


def test_authority_graph_robustness_t2_outputs_are_deterministic_across_insertion_order() -> (
    None
):
    edges = (
        AuthorityEdge(
    ...
    proof_a = graph_a.prove_reachability(
        "subject:alice", AuthorityScope("tenant-a", "client.read")
    )
    proof_b = graph_b.prove_reachability(
        "subject:alice", AuthorityScope("tenant-a", "client.read")
    )

    assert closure_a.scopes == closure_b.scopes
    assert tuple(path.edge_ids for path in proof_a.paths) == tuple(
        path.edge_ids for path in proof_b.paths
    )
