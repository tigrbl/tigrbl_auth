...
def test_layer_40_contains_only_registered_multi_component_use_cases() -> None:
    ...
    registered = {item["package"]: item["purpose"] for item in registry["capabilities"]}
    assert registered == CAPABILITY_PURPOSES
    assert all(purpose.startswith("coordinate ") for purpose in registered.values())
