...
BACKEND_APP_PACKAGES = {
    "public-app": (
        ...
        ROOT / "pkgs/90-backend-apps/tigrbl-auth-backend-app-public/src",
    ),
    "platform-admin-app": (
        "tigrbl_auth_backend_app_platform_admin",
        ROOT / "pkgs/90-backend-apps/tigrbl-auth-backend-app-platform-admin/src",
    ),
    "tenant-admin-app": (
        "tigrbl_auth_backend_app_tenant_admin",
        ROOT / "pkgs/90-backend-apps/tigrbl-auth-backend-app-tenant-admin/src",
    ),
    "developer-app": (
        "tigrbl_auth_backend_app_developer",
        ROOT / "pkgs/90-backend-apps/tigrbl-auth-backend-app-developer/src",
    ),
    "service-admin-app": (
        "tigrbl_auth_backend_app_service_admin",
        ROOT / "pkgs/90-backend-apps/tigrbl-auth-backend-app-service-admin/src",
    ),
    "resource-validation-app": (
        "tigrbl_auth_backend_app_resource_validation",
        ROOT / "pkgs/90-backend-apps/tigrbl-auth-backend-app-resource-validation/src",
    ),
}
...
def _import_api_package(product_surface: str):
    module_name, src_path = BACKEND_APP_PACKAGES[product_surface]
    src = str(src_path)
...
def test_backend_app_packages_build_surface_constrained_asgi_apps(
...
def test_public_and_resource_validation_surfaces_do_not_enable_control_plane() -> None:
    ...
    validation = resolve_deployment(
        profile="production", product_surface="resource-validation-app"
    )
...
def test_platform_admin_surface_owns_tenant_lifecycle_without_public_login_routes() -> (
    ...
    deployment = resolve_deployment(product_surface="platform-admin-app")
...
def test_tenant_admin_surface_excludes_platform_tenant_lifecycle() -> None:
    deployment = resolve_deployment(product_surface="tenant-admin-app")
...
def test_developer_and_service_admin_surfaces_are_product_filtered() -> None:
    developer = resolve_deployment(
        profile="production", product_surface="developer-app"
    )
    service = resolve_deployment(
        profile="production", product_surface="service-admin-app"
    )
...
async def test_product_apps_fail_closed_for_cross_surface_paths(tmp_path: Path) -> None:
    public = _import_api_package("public-app").build_app(_settings(tmp_path))
    platform = _import_api_package("platform-admin-app").build_app(_settings(tmp_path))
    tenant = _import_api_package("tenant-admin-app").build_app(_settings(tmp_path))
