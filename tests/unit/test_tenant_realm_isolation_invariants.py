...
def test_tenant_realm_isolation_t1_detects_cross_tenant_and_cross_realm_paths() -> None:
    graph = AuthorityDerivationGraph(
        nodes=(
            ...
            AuthorityNode(
                "authority:a", "authority", tenant_id="tenant-a", realm="realm-a"
            ),
            AuthorityNode(
                "authority:b", "authority", tenant_id="tenant-b", realm="realm-b"
            ),
        ),
        edges=(
            AuthorityEdge(
            ...
            AuthorityEdge(
                ...
                "authority:b",
                "grant",
                (AuthorityScope("tenant-b", "client.read", realm="realm-b"),),
            ),
        ),
    )
