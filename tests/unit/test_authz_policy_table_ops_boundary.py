...
import pytest
from tigrbl import first_table_record, list_table_records
...
def test_authz_policy_store_facade_is_removed() -> None:
    assert (
        importlib.util.find_spec("tigrbl_identity_storage.authz_policy_store") is None
    )
    assert not (
        ROOT
        / "pkgs"
...
@pytest.mark.asyncio
async def test_abac_policy_condition_helpers_are_runtime_owned(
    administrator_storage,
) -> None:
    row, conditions = await _runtime_op(
        upsert_attribute_policy,
        administrator_storage,
        name="tenant-risk",
        tenant_id="tenant-a",
        ...
        dynamic_conditions=({"field_name": "risk", "operator": "lte", "expected": 2},),
    )
    ...
    assert [condition["field_name"] for condition in active_conditions] == [
        "device_trust"
    ]


@pytest.mark.asyncio
async def test_delegated_admin_scope_ops_are_runtime_owned(
    administrator_storage,
) -> None:
    granted = await _runtime_op(
        grant_delegated_scope,
        administrator_storage,
        subject="delegate",
        tenant_ids=["tenant-a"],
    ...
    looked_up = await first_table_record(
        DelegatedAdminScope, administrator_storage, {"subject": "delegate"}
    )

    assert looked_up == granted
    assert [
        row["subject"]
        for row in await list_table_records(
            DelegatedAdminScope, administrator_storage, {"status": "active"}
        )
    ] == ["delegate"]

    revoked = await _runtime_op(
        revoke_delegated_scope, administrator_storage, subject="delegate"
    )
