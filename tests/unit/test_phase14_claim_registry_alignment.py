...
def test_phase14_claim_registry_verifier_passes() -> None:
...
def test_phase14_fapi2_security_profile_is_declared_and_enforced() -> None:
...
def test_phase14_claim_registry_imports_fapi_atomic_claims() -> None:
...
def test_phase14_repository_state_tracks_claim_model_completion() -> None:
    repository_state = _load_yaml("compliance/claims/repository-state.yaml")["repository_state"]
    assert repository_state["phase_14_claim_registry_canonical_complete"] is True
    assert repository_state["phase_14_fapi2_security_profile_declared_complete"] is True
    assert repository_state["phase_14_release_claims_machine_derivable"] is True
    assert repository_state["phase_14_core_targets_missing_from_feature_map"] == 0
    assert repository_state["phase_14_extension_targets_missing_from_feature_map"] == 0
    assert repository_state["phase_14_settings_backed_flags_missing_from_flag_map"] == 0
