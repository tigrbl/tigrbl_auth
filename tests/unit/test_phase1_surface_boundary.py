...
def test_phase1_surface_boundary_t0_inventory_classifies_all_admin_public_api_and_uix_surfaces():
    manifest = phase1_surface_boundary_manifest()
    registry = surface_registry()
    admin_uix_pkg = json.loads((ROOT / "apps/admin-uix/package.json").read_text())
...
def test_phase1_surface_boundary_t1_composes_public_and_admin_contracts_by_profile():
...
@pytest.mark.asyncio
async def test_phase1_surface_boundary_t2_fails_closed_for_surface_leaks_and_admin_gate(tmp_path):
    integrity = phase1_surface_boundary_integrity()
