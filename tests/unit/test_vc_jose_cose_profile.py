...
from tigrbl_credential_profile_vc_jose_cose import (
    CURRENT_VERSION,
    select_format,
    validate_cose_vc,
    validate_jose_vc,
...
def test_vc_jose_cose_rejects_carrier_and_artifact_confusion() -> None:
    validate_jose_vc({"alg": "ES256", "typ": "vc+jwt"}, VC)
    validate_cose_vc({1: -7}, VC)
    ...
    with pytest.raises(ValueError):
        validate_jose_vc({"alg": "none"}, VC)
    with pytest.raises(ValueError):
        validate_jose_vc({"alg": "ES256"}, {**VC, "vc": VC})