...
from tigrbl_identity_cli.discovery_service import diff_discovery, show_discovery
...
def test_operator_service_layer_round_trip(tmp_path: Path) -> None:
    ...
    create = create_resource(
        tenant_ctx, record_id="tenant-a", patch={"name": "Tenant A"}, if_exists="error"
    )
    assert create.status == "created"
    assert create.record and create.record["data"]["name"] == "Tenant A"

    update = update_resource(
        _context(tmp_path, "tenant", "tenant.update"),
        record_id="tenant-a",
        patch={"display_name": "Tenant Alpha"},
        if_missing="error",
    )
    assert update.status == "updated"
    fetched = get_resource(
        _context(tmp_path, "tenant", "tenant.get"), record_id="tenant-a"
    )
    ...
    export_result = run_export(
        _context(tmp_path, "export", "export.run"), output_path=export_path, redact=True
    )
    ...
    import_result = run_import(
        _context(imported_root, "import", "import.run"), path=export_path
    )
    assert import_result.status == "imported"
    imported = get_resource(
        _context(imported_root, "tenant", "tenant.get"), record_id="tenant-a"
    )
    assert imported.record and imported.record["data"]["display_name"] == "Tenant Alpha"

    delete = delete_resource(
        _context(imported_root, "tenant", "tenant.delete"), record_id="tenant-a"
    )
...
def test_runtime_observation_and_discovery(repo_root: Path | None = None) -> None:
    ...
    observe = observe_token_response(
        temp_root,
        access_token="abc.def.ghi",
        details={"subject": "user-1", "scope": "openid"},
    )
    assert observe["access_token_id"] == token_hash("abc.def.ghi")
    listing = list_resource_result(
        _context(temp_root, "token", "token.list"), limit=10, offset=0
    )
    assert listing.items and listing.items[0]["id"] == token_hash("abc.def.ghi")

    export_plan = validate_export_plan(
        _context(temp_root, "export", "export.validate"), redact=True
    )
    ...
    diff = diff_discovery(
        ...
        left_profile="baseline",
        right_profile="production",
    )
