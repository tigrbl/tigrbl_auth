...
|---|---|---|---|
| `public-uix` | tenant human user | login and end-user auth experience | current `apps/public-uix` |
| `platform-admin-uix` | platform superuser | deployment-wide control-plane administration | split from current `apps/admin-uix` |
| `tenant-admin-uix` | tenant admin | tenant-scoped administration | split from current `apps/admin-uix` |
...
| Current repo basis | `apps/public-uix/App.tsx`, discovery service, nginx allowlist |
...
| Current repo basis | `apps/admin-uix` auth shell plus tenant and identity management slices |
...
|---|---|---|
| `apps/public-uix` | `public-uix` | preserve as the end-user auth UIX |
