...
2. Install the published dependency stack declared in `pyproject.toml`, preserving resolver state with `uv.lock` plus the runner/storage extras required by each tox lane.
