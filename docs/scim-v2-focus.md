...
| Runtime helper | `ScimProvisioningPlane` exists under governance-extension code and supports schema registration, user provision, group provision, simple patch, tenant snapshots. | [`governance_extension.py`](../pkgs/40-capabilities/tigrbl-authz-policy/src/tigrbl_authz_policy/governance_extension.py) | Useful prototype/behavior seed, not a SCIM protocol server. |
