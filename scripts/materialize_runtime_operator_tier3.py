...
def _capability_scoped_text(text: str) -> str:
    ...
    text = re.sub(rf"{chronology_key}(\d+)_targeted_test_output", "retained_boundary_runtime_operator_test_output", text, flags=re.IGNORECASE)
    text = re.sub(rf"{chronology_key}(\d+)-targeted-test-output", "retained-boundary-runtime-operator-test-output", text, flags=re.IGNORECASE)
    return text
