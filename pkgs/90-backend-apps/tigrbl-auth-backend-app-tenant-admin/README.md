...
| Property | Value |
| --- | --- |
| Product surface | `tenant-admin-app` |
...
```python
from tigrbl_auth_backend_app_tenant_admin import app, build_app
```
...
```bash
docker compose -f docker/docker-compose.tenant-admin-app.yml up -d --build
```
