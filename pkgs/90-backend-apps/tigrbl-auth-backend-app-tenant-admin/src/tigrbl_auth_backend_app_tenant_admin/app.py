"""ASGI entrypoint for the tenant admin Tigrbl Auth backend application."""
...
from tigrbl_auth_backend_app_tenant_admin.contract import (
...
DEFAULT_CORS_ORIGINS = ("http://localhost:3012", "http://127.0.0.1:3012")


class TenantAdminCors:
    """Small ASGI CORS wrapper for the standalone tenant admin UIX demo."""
...
def build_app(settings_obj: object | None = None) -> "TigrblApp":
    ...
    return TenantAdminCors(
        build_product_app(
            PRODUCT_SURFACE, settings_obj, contract=TENANT_ADMIN_BACKEND_APP_CONTRACT
        )
    )
