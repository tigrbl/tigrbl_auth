...
| Property | Value |
| --- | --- |
| Product surface | `resource-validation-app` |
...
```python
from tigrbl_auth_backend_app_resource_validation import app, build_app
```
...
```bash
docker compose -f docker/docker-compose.resource-validation-app.yml up -d --build
```
