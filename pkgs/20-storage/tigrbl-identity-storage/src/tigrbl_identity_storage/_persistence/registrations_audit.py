...
async def upsert_client_registration_async(
    *,
    ...
    async with _session() as db:
        row = await _first_handler_record(ClientRegistration, db, {"client_id": client_id})
        payload = {
            "client_id": client_id,
            "tenant_id": tenant_id or _field(row, "tenant_id"),
            "registration_metadata": metadata or _field(row, "registration_metadata"),
            "contacts": list(contacts) if contacts is not None else _field(row, "contacts"),
            "software_id": software_id or _field(row, "software_id"),
            "software_version": software_version or _field(row, "software_version"),
            "registration_access_token_hash": registration_access_token_hash
            or _field(row, "registration_access_token_hash"),
            "registration_client_uri": registration_client_uri or _field(row, "registration_client_uri"),
            "issued_at": _field(row, "issued_at") or datetime.now(timezone.utc),
        }
        if row is None:
            return await _create_handler_record(ClientRegistration, db, payload)
        return await _update_handler_record(ClientRegistration, db, _record_id(row), payload)
    ...
    *,
    ...
    async with _session() as db:
        ...
        if effective_tenant_id is None:
            tenants = await _list_handler_records(Tenant, db)
            if tenants:
                effective_tenant_id = _record_id(tenants[0])
