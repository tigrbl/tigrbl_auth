...
class DidDocument(RestOltpTable, GUIDPk, Timestamped):
    ...
    did: Mapped[str] = acol(storage=S(String(1000), nullable=False, unique=True, index=True))
    method: Mapped[str] = acol(storage=S(String(64), nullable=False, index=True))
    authoritative: Mapped[bool] = acol(storage=S(Boolean, nullable=False, default=False, index=True))
    current_version: Mapped[str | None] = acol(storage=S(String(128), nullable=True, index=True))
...
class DidDocumentVersion(RestOltpTable, GUIDPk, Timestamped):
    ...
    document_digest: Mapped[str] = acol(storage=S(String(128), nullable=False, index=True))
...
class DidVerificationMethod(RestOltpTable, GUIDPk, Timestamped):
    ...
    __table_args__ = ({'schema': 'authn'},)
    did: Mapped[str] = acol(storage=S(String(1000), nullable=False, index=True))
    method_id: Mapped[str] = acol(storage=S(String(1000), nullable=False, unique=True, index=True))
...
class DidService(RestOltpTable, GUIDPk, Timestamped):
    ...
    __table_args__ = ({'schema': 'authn'},)
    did: Mapped[str] = acol(storage=S(String(1000), nullable=False, index=True))
    service_id: Mapped[str] = acol(storage=S(String(1000), nullable=False, unique=True, index=True))
...
class DidResolutionCache(RestOltpTable, GUIDPk, Timestamped):
    ...
    __table_args__ = ({'schema': 'authn'},)
    did_url: Mapped[str] = acol(storage=S(String(1000), nullable=False, unique=True, index=True))
    result_digest: Mapped[str] = acol(storage=S(String(128), nullable=False, index=True))
    result: Mapped[dict] = acol(storage=S(JSON, nullable=False))
    resolved_at: Mapped[dt.datetime | None] = acol(storage=S(TZDateTime, nullable=True, index=True))
