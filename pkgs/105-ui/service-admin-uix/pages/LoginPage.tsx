...
export function LoginPage({ error, onLogin }: { error: string; onLogin: (username: string, email: string, tenantId: string) => Promise<void> }) {
  const [username, setUsername] = useState("service-admin");
  const [email, setEmail] = useState("service-admin@example.test");
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setLocalError(nextError instanceof Error ? nextError.message : "Unable to establish service admin session.");
    }
  }
