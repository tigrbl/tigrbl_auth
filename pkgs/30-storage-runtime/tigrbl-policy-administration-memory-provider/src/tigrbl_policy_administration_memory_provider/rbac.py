...
from tigrbl_authorization_policy_durability import RoleTable as _StoredRole
from tigrbl_authorization_policy_durability import (
    TenantMembershipTable as _StoredTenantMembership,
)
...
class RBACAdministrator:
    ...
    async def assign_role(
        self, subject: str, role_name: str, *, tenant_id: str | None = None
    ) -> None:
        if not tenant_id:
    ...
    async def assignments_for(
        ...
        return await _StoredTenantMembership.handlers.role_names_for_principal.core(
            {
                "payload": {"principal_id": subject, "tenant_id": tenant_id},
                "db": self.db,
            }
        )
    ...
    async def decide(
        self, subject: str, permission: str, tenant_id: str | None = None
    ) -> PolicyDecision:
        ...
        if any(_permission_matches(deny, permission) for deny in denies):
            return PolicyDecision(
                False, "permission denied by RBAC role assignments", matched_roles
            )
        if any(_permission_matches(grant, permission) for grant in grants):
            return PolicyDecision(
                True, "permission granted by assigned role", matched_roles
            )
        return PolicyDecision(
            False, "permission denied by RBAC role assignments", matched_roles
        )
    ...
    async def _decision_inputs(
        ...
        for (
            role_name
        ) in await _StoredTenantMembership.handlers.role_names_for_principal.core(
            {
                "payload": {"principal_id": subject, "tenant_id": tenant_id},
                "db": self.db,
            }
