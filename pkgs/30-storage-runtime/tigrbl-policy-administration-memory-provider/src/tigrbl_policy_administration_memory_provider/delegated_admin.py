...
from tigrbl_authorization_policy_durability import (
    DelegatedAdminScopeTable as _StoredDelegatedAdminScope,
)


def _delegated_scope_contract(row: Any) -> DelegatedAdminScope:
    visible = (
        _str_tuple(_row_value(row, "visible_client_fields"))
        or DELEGATED_VISIBLE_CLIENT_FIELDS
    )
    mutable = (
        _str_tuple(_row_value(row, "mutable_client_fields"))
        or DELEGATED_MUTABLE_CLIENT_FIELDS
    )
    return DelegatedAdminScope(
        ...
        service_identity_permissions=_str_tuple(
            _row_value(row, "service_identity_permissions")
        ),
    )
        ...
        row = await _StoredDelegatedAdminScope.handlers.grant_scope.core(
            {
                "payload": {
                    ...
                    "service_identity_permissions": list(
                        _str_tuple(service_identity_permissions)
                    ),
                },
                "db": self.db,
            }
        ...
        *,
        ...
        if tenant_id not in scope.tenant_ids:
            return PolicyDecision(
                False, "permission denied by delegated tenant scope", (scope.subject,)
            )
        if not any(
            _permission_matches(grant, permission) for grant in scope.permissions
        ):
            return PolicyDecision(
                False, "permission denied by delegated admin scope", (scope.subject,)
            )
        patch_field_set = set(patch_fields)
        if patch_field_set and not patch_field_set.issubset(
            set(scope.mutable_client_fields)
        ):
            return PolicyDecision(
                False,
                "permission denied by delegated client mutation scope",
                tuple(sorted(patch_field_set)),
            )
        return PolicyDecision(
            True, "permission granted by delegated admin scope", (scope.subject,)
        )

    async def visible_tenant_ids(
        self, subject: str, tenant_ids: Iterable[str]
    ) -> tuple[str, ...]:
        scope = await self.scope_for(subject)
        if scope is None:
        ...
        return tuple(
            sorted(
                tenant_id for tenant_id in tenant_ids if tenant_id in scope.tenant_ids
            )
        )
