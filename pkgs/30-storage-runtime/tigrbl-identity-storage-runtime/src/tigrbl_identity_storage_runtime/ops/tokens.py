...
async def persist_issued_token(ctx: Mapping[str, Any]) -> Any:
    ...
    record = {
        ...
        "credential_or_grant_reference": payload.get(
            "credential_or_grant_reference"
        )
        ...
        "kid": claims.get("kid")
        or payload.get("kid")
        or field_value(row, "kid"),
        "key_version": payload.get("key_version")
        or field_value(row, "key_version"),
...
async def revoke_refresh_token_family(ctx: Mapping[str, Any]) -> list[Any]:
    ...
    for row in rows:
        ...
        if (
            ...
            record["reuse_detected_at"] = (
                field_value(row, "reuse_detected_at") or now
            )
        updated.append(
