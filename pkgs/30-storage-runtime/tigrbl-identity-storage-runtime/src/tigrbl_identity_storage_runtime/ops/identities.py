...
def _identity_id(ctx: Mapping[str, Any]) -> Any:
    ...
    return payload.get("identity_id") or payload.get("user_id") or payload.get("id") or path.get("id")
...
async def replace_password_hash(ctx: Mapping[str, Any]) -> Any:
    ...
    return await update_table_record(
        ...
        {"password_hash": encoded_hash, "must_change_password": bool(payload.get("must_change_password", False))},
    )
