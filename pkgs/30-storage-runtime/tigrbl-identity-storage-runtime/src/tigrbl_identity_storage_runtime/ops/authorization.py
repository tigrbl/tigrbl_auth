    ...
    payload = {
        **filters,
        ...
        "denied_permissions": list(
            _strings(values.get("denied_permissions", ()))
        ),
...
async def list_roles_for_tenant(ctx: Mapping[str, Any]) -> tuple[object, ...]:
    ...
    return tuple(
        row
        for row in rows
        if field_value(row, "tenant_id") in {None, tenant_id}
    )


async def upsert_attribute_policy(ctx: Mapping[str, Any]) -> tuple[object, list[object]]:
    values = dict(payload_from_context(ctx))
    db = database_from_context(ctx)
    filters = {
        "name": values["name"],
...
async def grant_delegated_scope(ctx: Mapping[str, Any]) -> object:
    ...
    existing = await first_table_record(
        DelegatedAdminScope, db, {"subject": subject}
    )
    if existing is None:
