...
from tigrbl_identity_durability import (
    ...
    CredentialMfaFactorTable as CredentialMfaFactor,
    CredentialRecoveryCodeTable as CredentialRecoveryCode,
    CredentialTable as Credential,
)
from tigrbl_webauthn_durability import (
    WebAuthnCredentialTable as CredentialWebAuthnPasskey,
)
...
class AdvancedAuthenticatorRegistry:
    ...
    def revoke_passwordless_credential(
        ...
        *,
        ...
        if decision.step_up_required:
            allowed_methods.update(
                ...
                if factor.subject_id == subject_id
                and factor.tenant_id == tenant_id
                and not factor.revoked
            )
    ...
    def complete_passwordless_assertion(
        ...
        *,
        ...
        if credential_id in self._webauthn_credentials:
            ...
            if (
                credential.subject_id != challenge.subject_id
                or credential.tenant_id != challenge.tenant_id
            ):
    ...
    def complete_mfa_challenge(
        ...
        *,
        ...
        if (
            factor.subject_id != challenge.subject_id
            or factor.tenant_id != challenge.tenant_id
        ):
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "active_passwordless_credentials": sum(
                ...
                for credential in self._passwordless_credentials.values()
            ),
            "active_webauthn_credentials": sum(
                not credential.revoked
                for credential in self._webauthn_credentials.values()
            ),
            "active_mfa_factors": sum(
                not factor.revoked for factor in self._mfa_factors.values()
            ),
            "table_owned": True,
