...
def sig_structure(context: str, protected: bytes, external_aad: bytes, payload: bytes, *, signer_protected: bytes | None = None) -> bytes:
    values=[context, protected]
    if signer_protected is not None: values.append(signer_protected)
...
def mac_structure(context: str, protected: bytes, external_aad: bytes, payload: bytes) -> bytes:
