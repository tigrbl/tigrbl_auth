"""Serialization and validation for final OpenID Identity Assurance payloads."""
...
OIDC_SPEC_URL = "https://openid.net/specs/openid-connect-4-identity-assurance-1_0-final.html"
...
def parse_verified_claims(value: Mapping[str, Any]) -> VerifiedClaims:
    ...
    if not isinstance(verification, Mapping) or not isinstance(claims, Mapping):
        raise ValueError("verified_claims requires object-valued verification and claims")
    ...
    for item in raw_evidence:
        ...
        evidence.append(AssuranceEvidence(item["type"], {k: v for k, v in item.items() if k != "type"}))
    known = {"trust_framework", "assurance_level", "assurance_process", "time", "verification_process", "evidence"}
    ...
    # Unknown members are intentionally ignored as required by the schema.
    _ = known
    return VerifiedClaims(metadata, dict(claims))


def serialize_verified_claims(value: VerifiedClaims) -> dict[str, Any]:
    verification: dict[str, Any] = {"trust_framework": value.verification.trust_framework}
    for name in ("assurance_level", "assurance_process", "time", "verification_process"):
        member = getattr(value.verification, name)
        if member is not None:
            verification[name] = member
    if value.verification.evidence:
        verification["evidence"] = [
            {"type": item.type, **dict(item.attributes)} for item in value.verification.evidence
        ]
