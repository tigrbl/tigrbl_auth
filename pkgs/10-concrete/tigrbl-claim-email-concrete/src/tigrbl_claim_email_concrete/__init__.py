...
class EmailClaim(ClaimBase):
    ...
    def validate_value(cls, value):
        if not (isinstance(value, str) and '@' in value and not value.startswith('@') and not value.endswith('@')):
            raise ValueError("email has an invalid OIDC claim value")
