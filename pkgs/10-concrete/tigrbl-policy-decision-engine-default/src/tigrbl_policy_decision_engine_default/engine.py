...
class PolicyDecisionEngine:
    ...
    def decide_rbac(self, request: PolicyRequest) -> PolicyDecision:
        ...
        for policy in self.roles:
            ...
            if policy.role not in request.roles:
                continue
            if any(
                matches_dotted_pattern(permission, request.action)
        ...
        if matched_deny:
            return PolicyDecision(
                False, "denied by RBAC role policy", tuple(sorted(matched_deny))
            )
        if matched_allow:
            return PolicyDecision(
                True, "allowed by RBAC role policy", tuple(sorted(matched_allow))
            )
    ...
    def decide_abac(self, request: PolicyRequest) -> PolicyDecision:
        ...
        if matched_deny:
            return PolicyDecision(
                False, "denied by ABAC attribute policy", tuple(sorted(matched_deny))
            )
        if matched_allow:
            return PolicyDecision(
                True, "allowed by ABAC attribute policy", tuple(sorted(matched_allow))
            )
    ...
    def decide_pbac(self, request: PolicyRequest) -> PolicyDecision:
        ...
        if matched_deny:
            return PolicyDecision(
                False, "denied by PBAC permission policy", tuple(sorted(matched_deny))
            )
        if matched_allow:
            return PolicyDecision(
                True, "allowed by PBAC permission policy", tuple(sorted(matched_allow))
            )
    ...
    def decide_delegation(self, request: PolicyRequest) -> PolicyDecision:
        ...
        for policy in self.delegations:
            if (
                policy.delegate != request.subject
                or policy.delegator != request.delegated_by
            ):
                continue
            if request.tenant_id not in policy.tenant_ids:
                return PolicyDecision(
                    False, "delegation tenant scope denied", (policy.delegator,)
                )
            if any(
                matches_dotted_pattern(action, request.action)
                for action in policy.actions
            ):
                return PolicyDecision(
                    True, "allowed by delegation policy", (policy.delegator,)
                )
    ...
    def decide_admin(self, request: PolicyRequest) -> PolicyDecision:
        ...
        for policy in self.admins:
            ...
            if policy.superuser:
                return PolicyDecision(
                    True, "allowed by superuser admin policy", (policy.subject,)
                )
            if request.tenant_id in policy.tenant_ids and any(
                ...
                return PolicyDecision(
                    True, "allowed by tenant admin policy", (policy.subject,)
                )
    ...
    def evaluate(self, request: PolicyRequest) -> PolicyDecision:
        ...
        hard_denies = [
            ...
            and kind in {PolicyKind.DELEGATION, PolicyKind.ADMIN}
        ]
        allow_decisions = [
            (kind, decision)
            for kind, decision in decisions
            if decision.allowed
            and kind in {PolicyKind.RBAC, PolicyKind.ABAC, PolicyKind.PBAC}
        ]
        if hard_denies:
            allowed = False
