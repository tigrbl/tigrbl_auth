...
class PhoneNumberVerifiedClaim(ClaimBase):
    claim_name = "phone_number_verified"
    default_claim_type = ClaimType.ASSURANCE
    default_value_type = ClaimValueType.BOOLEAN
    default_standards = ("OIDC Core 1.0",)

    @classmethod
    def validate_value(cls, value):
        if not (isinstance(value, bool)):
            raise ValueError("phone_number_verified has an invalid OIDC claim value")


__all__ = ["PhoneNumberVerifiedClaim"]
