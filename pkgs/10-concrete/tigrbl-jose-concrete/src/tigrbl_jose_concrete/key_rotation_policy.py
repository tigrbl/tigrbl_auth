"""Deterministic signing-key rotation policy and administration behavior."""
...
class KeyRotationPolicyGovernance(KeyRotationPolicyBase):
    ...
    def approve_policy_version(
        ...
        approved = replace(
            policy, status="approved", approved_by=actor, approved_at=utc_now_iso()
        )
    ...
    def effective_policy(
        ...
        *,
        ...
        ref = self._published.get(
            (tenant_id, issuer, profile, key_class, key_use, algorithm)
        )
        if ref is None:
            return None
    ...
    def require_effective_policy(self, **scope: str) -> EffectiveKeyRotationPolicy:
        ...
        if policy is None:
            raise PermissionError(
                "key rotation requires an effective governance policy"
            )
        return policy
    ...
    def assert_administration_does_not_own_policy(self, administration: object) -> None:
        ...
        if overlap:
            raise KeyRotationPolicyOverlapError(
                "administration plane exposes governance policy authority"
            )
...
class KeyRotationAdministration(KeyRotationAdministrationBase):
    ...
    def execute_rotation(
        ...
        *,
        ...
        if not retired:
            raise PermissionError(
                "rotation policy requires old key retirement evidence"
            )
