...
def decode_tagged_cwt(encoded: bytes, *, require_tag: bool = True) -> CwtClaimsSet:
    ...
    if isinstance(value, cbor2.CBORTag):
        if value.tag != CWT_TAG: raise ValueError(f"unexpected CBOR tag: {value.tag}")
        value=value.value
    elif require_tag: raise ValueError("CWT tag 61 is required")
    if not isinstance(value, Mapping): raise ValueError("CWT payload must be a map")
