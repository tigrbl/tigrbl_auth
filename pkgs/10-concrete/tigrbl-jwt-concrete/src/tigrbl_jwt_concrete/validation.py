    ...
    *,
    ...
    for name in ("exp", "nbf", "iat"):
        if name in claims and (
            isinstance(claims[name], bool) or not isinstance(claims[name], (int, float))
        ):
            raise ValueError(f"JWT {name} must be NumericDate")
    if "exp" in claims and current > float(claims["exp"]) + clock_skew:
        raise ValueError("JWT expired")
    if "nbf" in claims and current + clock_skew < float(claims["nbf"]):
        raise ValueError("JWT is not active")
    if "iat" in claims and float(claims["iat"]) > current + clock_skew:
    ...
    if audience is not None:
        ...
        values = (
            ...
            else tuple(presented)
            if isinstance(presented, Sequence)
            else ()
        )
