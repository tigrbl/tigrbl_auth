...
def parse_jwe_json(value: str | bytes | Mapping[str, object]) -> JweJson:
    ...
    for item in raw_recipients:
        ...
        if (
            set(protected).intersection(shared)
            or set(protected).intersection(header)
            or set(shared).intersection(header)
        ):
            raise ValueError("JWE header parameters must be disjoint")
        recipients.append(
            JweRecipient(dict(header), base64url_decode(item.get("encrypted_key", "")))
        )
    return JweJson(
        dict(protected),
        dict(shared),
        tuple(recipients),
        base64url_decode(obj["iv"]),
        base64url_decode(obj["ciphertext"]),
        base64url_decode(obj["tag"]),
        base64url_decode(obj["aad"]) if "aad" in obj else None,
    )
