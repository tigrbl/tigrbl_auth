...
_PASSWORD_HASHER = BcryptSecretHasher()
...
class PasswordBackend:
    ...
    async def authenticate(self, db: Any, identifier: str, password: str) -> Any:
        ...
        if (
            not row
            or not _PASSWORD_HASHER.verify_secret(password, row.password_hash).verified
        ):
...
class ApiKeyBackend:
    ...
    async def _get_service_key_row(self, db: Any, digest: str) -> Any | None:
    ...
    async def authenticate(self, db: Any, api_key: str) -> tuple[PrincipalLike, str]:
        ...
        user = await self._resolve_user(db, key_row) if key_row is not None else None
        if key_row and user:
