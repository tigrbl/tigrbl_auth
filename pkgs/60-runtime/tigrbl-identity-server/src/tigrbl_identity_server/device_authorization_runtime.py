...
async def device_authorization_request(*, request, db):
    ...
    if resources:
        if not deployment.flag_enabled("enable_rfc8707"):
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "resource indicators disabled"
            )
        try:
...
async def device_authorization(request: Any, db: Any) -> Any:
    ...
    payload = (
        ...
        else getattr(result, "model_dump", lambda **_: {})(mode="json")
    )
    observe_device_authorization_response(
        _repo_root(), device_code=payload.get("device_code"), details=payload
    )
    return result
