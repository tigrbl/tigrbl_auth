...
from .crud_compat import (
    ...
    _tigrbl,
    _deleted_count,
    _is_local_table_model,
    _normalize_payload,
    _row_items,
    _table_replace_payload,
)
from .handler_compat import (
            ...
            async def _rpc_clear(
                ...
                *,
                ...
                if _deleted_count(result) > 0 or not normalized:
                    return result
                rows = _row_items(await _crud_ops.list(_model, normalized, db=db))
                deleted = 0
                for row in rows:
