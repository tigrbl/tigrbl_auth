"""Authorization API revision compatibility paths."""

from dataclasses import dataclass

from .versions import CURRENT_VERSION, VERSION_HISTORY


@dataclass(frozen=True, slots=True)
class AuthzenCompatibility:
    source: str
    target: str
    compatible: bool
    lossless: bool
    migration_required: bool


COMPATIBILITY_PATHS = tuple(
    AuthzenCompatibility(
        version.identifier,
        CURRENT_VERSION.identifier,
        True,
        True,
        version.identifier != CURRENT_VERSION.identifier,
    )
    for version in VERSION_HISTORY
)


def compatibility(
    source: str, target: str = CURRENT_VERSION.identifier
) -> AuthzenCompatibility:
    for path in COMPATIBILITY_PATHS:
        if path.source == source and path.target == target:
            return path
    return AuthzenCompatibility(source, target, False, False, False)


__all__ = ["COMPATIBILITY_PATHS", "AuthzenCompatibility", "compatibility"]
