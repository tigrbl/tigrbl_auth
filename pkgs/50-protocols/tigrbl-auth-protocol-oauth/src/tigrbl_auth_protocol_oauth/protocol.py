...
class OAuthStateOperations:
    persist_client: Callable[[OAuthClient], None]
    resolve_client: Callable[[str], OAuthClient | None]
    persist_device_authorization: Callable[[DeviceAuthorization], None]
    resolve_device_authorization: Callable[[str], DeviceAuthorization | None]
    reserve_dpop_jti: Callable[[str, str], bool]


class InMemoryOAuthState:
    ...
    def operations(self) -> OAuthStateOperations:
        return OAuthStateOperations(
            self.save_client,
            self.get_client,
            self.save_device_authorization,
            self.get_device_authorization,
            self.remember_dpop_jti,
        )


InMemoryOAuthRepository = InMemoryOAuthState


class OAuthProtocolService:
    def __init__(
        self,
        operations: OAuthStateOperations | None = None,
        now: Callable[[], datetime] = _utc_now,
        *,
        repository: InMemoryOAuthState | None = None,
    ) -> None:
        if operations is None:
            if repository is None:
                raise TypeError("OAuth state operations are required")
    ...
    def register_client(self, client: OAuthClient) -> OAuthClient:
        if not client.client_id or not client.tenant_id:
            raise OAuthError(
                "invalid_client_metadata", "client_id and tenant_id are required"
            )
        if not client.allowed_scopes:
            raise OAuthError(
                "invalid_client_metadata", "at least one allowed scope is required"
            )
        self.operations.persist_client(client)
        return client
    ...
    def start_device_authorization(
        ...
        *,
        ...
        if expires_in_seconds <= 0 or interval_seconds <= 0:
            raise OAuthError(
                "invalid_request",
                "device code expiry and polling interval must be positive",
            )
        ...
        self.operations.persist_device_authorization(grant)
        return grant

    def approve_device_authorization(
        self, device_code: str, *, subject: str
    ) -> DeviceAuthorization:
        ...
        self.operations.persist_device_authorization(approved)
        return approved
    ...
    def poll_device_authorization(self, device_code: str) -> TokenExchangeResult:
        ...
        if self.now() >= grant.expires_at:
            ...
            self.operations.persist_device_authorization(expired)
        ...
        if grant.status == OAuthGrantStatus.PENDING:
            ...
            self.operations.persist_device_authorization(polled)
        ...
        self.operations.persist_device_authorization(consumed)
    ...
    def exchange_token(
        ...
        *,
        ...
        if (
            subject_token_subject != requested_subject
            and actor != subject_token_subject
        ):
            raise OAuthError(
                "invalid_request", "token exchange actor must match the subject token"
            )
        return TokenExchangeResult(
    ...
    def validate_dpop(
        ...
        *,
        ...
        if client.jwk_thumbprint and proof.jwk_thumbprint != client.jwk_thumbprint:
            raise OAuthError(
                "invalid_dpop_proof",
                "DPoP key thumbprint does not match client binding",
            )
        ...
        if abs(now_seconds - int(proof.iat)) > max_age_seconds:
            raise OAuthError(
                "invalid_dpop_proof", "DPoP proof is outside the accepted time window"
            )
        if not self.operations.reserve_dpop_jti(client_id, proof.jti):
            raise OAuthError("use_dpop_nonce", "DPoP proof replay detected")

    def validate_mtls_client(
        self, *, client_id: str, certificate_thumbprint: str
    ) -> None:
        client = self._require_client(client_id)
    ...
    def _require_client(self, client_id: str) -> OAuthClient:
        client = self.operations.resolve_client(client_id)
    ...
    def _require_device_authorization(self, device_code: str) -> DeviceAuthorization:
        grant = self.operations.resolve_device_authorization(device_code)
        if grant is None:
    ...
    def _require_scopes(client: OAuthClient, scopes: tuple[str, ...]) -> None:
        ...
        if not scopes or not set(scopes).issubset(allowed):
            raise OAuthError(
                "invalid_scope", "requested scopes are not allowed for client"
            )
