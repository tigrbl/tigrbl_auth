...
def _utc(value: Any) -> datetime:
    ...
    raise PushedAuthorizationRequestError(
        f"invalid PAR timestamp value: {RFC9126_SPEC_URL}"
    )
...
def validate_request_uri(value: str | None) -> str:
    ...
    if not is_request_uri(request_uri):
        raise PushedAuthorizationRequestError(
            f"invalid request_uri value: {RFC9126_SPEC_URL}"
        )
    return request_uri
...
def validate_pushed_authorization_request_row(
    ...
    *,
    ...
    effective_request_uri = validate_request_uri(
        request_uri or getattr(row, "request_uri", None)
    )
    row_request_uri = validate_request_uri(getattr(row, "request_uri", None))
    if row_request_uri != effective_request_uri:
        raise PushedAuthorizationRequestError(
            f"request_uri mismatch: {RFC9126_SPEC_URL}"
        )
    ...
    if expires_at <= current:
        raise PushedAuthorizationRequestError(
            f"request_uri has expired: {RFC9126_SPEC_URL}"
        )
    if PAR_ONE_TIME_USE and consumed_at is not None:
        raise PushedAuthorizationRequestError(
            f"request_uri has already been consumed: {RFC9126_SPEC_URL}"
        )
    bound_client = getattr(row, "client_id", None)
    if (
        client_id not in {None, ""}
        and bound_client not in {None, "", client_id}
        and str(bound_client) != str(client_id)
    ):
        raise PushedAuthorizationRequestError(
            f"request_uri is not bound to the presented client_id: {RFC9126_SPEC_URL}"
        )
...
def consume_pushed_authorization_request(
    row: Any, *, now: datetime | None = None
) -> datetime:
    consumed_at = now if now is not None else datetime.now(timezone.utc)
    consumed_at = (
        consumed_at
        if consumed_at.tzinfo is not None
        else consumed_at.replace(tzinfo=timezone.utc)
    )
