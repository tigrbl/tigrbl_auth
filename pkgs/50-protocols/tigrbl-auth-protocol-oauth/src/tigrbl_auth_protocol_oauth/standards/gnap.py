"""Core GNAP transaction request/continuation structural models."""
...
def parse_gnap_request(value: Mapping[str, Any]) -> GnapRequest:
    ...
    if isinstance(access, Mapping):
        access = [access]
    if not isinstance(access, list) or not access or not all(isinstance(item, Mapping) for item in access):
