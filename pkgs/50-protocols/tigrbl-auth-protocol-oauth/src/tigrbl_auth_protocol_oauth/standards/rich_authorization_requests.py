...
def _normalize_locations(value: Any) -> list[str] | None:
    ...
    if isinstance(value, str):
        ...
        raise ValueError(
            f"authorization_details locations must be a string or array: {RFC9396_SPEC_URL}"
        )
    normalized = []
    for item in items:
        if not _is_absolute_uri(item):
            raise ValueError(
                f"authorization_details locations must be absolute URIs: {RFC9396_SPEC_URL}"
            )
...
def _normalize_multi_string(name: str, value: Any) -> list[str] | None:
    ...
    if isinstance(value, str):
        ...
        raise ValueError(
            f"authorization_details {name} must be a string or array: {RFC9396_SPEC_URL}"
        )
    ...
    for key in ("resource", "resource_indicator"):
        if key in payload and payload[key] not in {None, ""}:
            ...
            if not _is_absolute_uri(value):
                raise ValueError(
                    f"authorization_details {key} must be an absolute URI: {RFC9396_SPEC_URL}"
                )
            payload[key] = value
        else:
            payload.pop(key, None)
...
def bind_authorization_details(
    ...
    *,
    ...
    for item in details:
        detail = (
            item
            if isinstance(item, AuthorizationDetail)
            else AuthorizationDetail.model_validate(item)
        )
        ...
        if detail_targets:
            try:
                selection = select_resource_indicator(
                    detail_targets, audience=effective_target
                )
            except ValueError as exc:
                raise ValueError(
                    f"authorization_details resource binding is inconsistent: {RFC9396_SPEC_URL}"
                ) from exc
    ...
    return AuthorizationDetailsBinding(
        details=bound_details, resource=effective_target, audience=effective_target
    )


def normalize_authorization_details(
    raw: str | list[dict[str, Any]] | dict[str, Any],
    *,
    resource: str | None = None,
    audience: str | None = None,
) -> AuthorizationDetailsBinding:
