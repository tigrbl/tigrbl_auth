...
def validate_id_token_profile(headers: Mapping[str, object], claims: Mapping[str, object], *, expected_issuer: str, client_id: str, expected_nonce: str | None = None, now: int | None = None, clock_skew: int = 0) -> None:
    ...
    audience = claims["aud"]
    audiences = (audience,) if isinstance(audience, str) else tuple(audience) if isinstance(audience, Sequence) else ()
    ...
    if not isinstance(claims["exp"], (int, float)) or current > claims["exp"] + clock_skew:
        raise ValueError("ID Token has expired")
    if not isinstance(claims["iat"], (int, float)) or claims["iat"] > current + clock_skew:
