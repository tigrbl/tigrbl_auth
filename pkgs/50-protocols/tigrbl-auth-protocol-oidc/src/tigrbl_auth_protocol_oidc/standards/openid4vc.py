...
def parse_credential_offer(value: Mapping[str, Any]) -> CredentialOffer:
    ...
    if not isinstance(ids, list) or not ids or not all(isinstance(i, str) and i for i in ids):
        raise ValueError("credential_configuration_ids must be a non-empty string array")
...
def parse_presentation_request(value: Mapping[str, Any]) -> PresentationRequest:
    ...
    if not isinstance(client_id, str) or not client_id or not isinstance(nonce, str) or not nonce:
    ...
    return PresentationRequest(client_id, nonce, "vp_token", value.get("response_mode"), query)
