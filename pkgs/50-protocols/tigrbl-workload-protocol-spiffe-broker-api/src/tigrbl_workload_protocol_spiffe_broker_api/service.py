...
class BrokerApiService:
    def __init__(
        self,
        *,
        authorize: Callable[[object], bool],
        ...
        if unknown or missing:
            raise ValueError(
                f"Broker API handler mismatch; unknown={sorted(unknown)}, missing={sorted(missing)}"
            )
    ...
    def dispatch(self, actor: object, rpc: str, request: object) -> object:
        if not self._authorize(actor):
            raise PermissionError("broker caller is not authorized")
        if rpc not in RPC_NAMES:
            raise ValueError(f"unknown Broker API RPC: {rpc}")
