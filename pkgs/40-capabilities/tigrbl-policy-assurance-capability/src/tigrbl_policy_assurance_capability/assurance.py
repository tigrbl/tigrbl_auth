...
from tigrbl_authority_graph_concrete import AuthorityDerivationGraph
...
class AuthorizationSafetyPropertyEvaluator:
    ...
    def evaluate(
        self, evaluations: Iterable[InvariantEvaluation]
    ) -> SafetyPropertyResult:
...
def validate_authorization_referential_integrity(
    *,
    ...
    for ref_id, ref_types in by_ref_id.items():
        if len(ref_types) > 1:
            failures.append(
                f"reference id {ref_id!r} is reused across incompatible types"
            )
    for target_id, ref_types in by_target_id.items():
        if len(ref_types) > 1:
            failures.append(
                f"target id {target_id!r} is referenced as incompatible types"
            )
    failures_tuple = tuple(sorted(set(failures)))
    return IntegrityReport(
        passed=not failures_tuple, failures=failures_tuple, checked_count=len(rows)
    )
...
def validate_trust_graph_integrity(
    *,
    ...
    for edge in rows:
        if edge.revoked:
            failures.append(
                f"trust edge {edge.edge_id!r} is revoked and cannot be active"
            )
        if edge.issuer not in subjects:
            failures.append(
                f"trust edge {edge.edge_id!r} has unknown issuer {edge.issuer!r}"
            )
        if edge.subject not in subjects:
            failures.append(
                f"trust edge {edge.edge_id!r} has unknown subject {edge.subject!r}"
            )
        if edge.tenant_id not in tenants:
            failures.append(
                f"trust edge {edge.edge_id!r} has unknown tenant {edge.tenant_id!r}"
            )
        if edge.trust_domain not in domains:
            failures.append(
                f"trust edge {edge.edge_id!r} has unknown trust domain {edge.trust_domain!r}"
            )
        if edge.subject_tenant_id and edge.subject_tenant_id != edge.tenant_id:
            failures.append(
                f"trust edge {edge.edge_id!r} crosses tenant {edge.tenant_id!r}->{edge.subject_tenant_id!r}"
            )
        if edge.subject_trust_domain:
            if edge.subject_trust_domain not in domains:
                failures.append(
                    f"trust edge {edge.edge_id!r} has unknown subject trust domain {edge.subject_trust_domain!r}"
                )
            if (
                edge.subject_trust_domain != edge.trust_domain
                and (edge.trust_domain, edge.subject_trust_domain)
                not in allowed_cross_domain
            ):
                failures.append(
                    f"trust edge {edge.edge_id!r} crosses trust domain {edge.trust_domain!r}->{edge.subject_trust_domain!r}"
                )
    ...
    for path in paths:
        for scope in path.scopes:
            if scope.tenant_id != expected_tenant_id:
                failures.append(
                    f"path {'/'.join(path.edge_ids)} crosses tenant {scope.tenant_id!r}"
                )
            if expected_realm and scope.realm and scope.realm != expected_realm:
                failures.append(
                    f"path {'/'.join(path.edge_ids)} crosses realm {scope.realm!r}"
                )
...
def build_control_plane_correctness_report(
    *,
    ...
    failures = tuple(
        [
            *(
                f"missing required proof section: {section_id}"
...
def replay_policy_determinism(
    *,
    ...
    for case in cases:
        ...
        if not stable:
            failures.append(
                f"case {case.case_id!r} produced non-deterministic decisions"
            )
        results.append(
            PolicyReplayResult(
...
def compare_policy_version_decisions(
    *,
    ...
    for case in cases:
        ...
        if not attributed:
            failures.append(
                f"case {case.case_id!r} changed without policy-version attribution"
            )
        changes.append(
            DecisionStabilityChange(
                case_id=case.case_id,
...
def evaluate_liveness_convergence(
    events: tuple[ConvergenceEvent, ...],
) -> LivenessConvergenceReport:
    by_id = {event.event_id: event for event in events}
    rows = tuple(by_id[event_id] for event_id in sorted(by_id))
    converged = tuple(
        event for event in rows if event.state is ConvergenceState.CONVERGED
    )
    ...
    failures = tuple(
        [
            *(
                f"mutation {event.mutation_id!r} is pending convergence"
                for event in pending
            )
        ]
        + [
            *(
                f"mutation {event.mutation_id!r} converged after its expected window"
                for event in late
            )
        ]
    )
