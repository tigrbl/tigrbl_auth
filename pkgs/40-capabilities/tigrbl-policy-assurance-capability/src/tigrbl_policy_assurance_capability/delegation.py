"""Delegation lifecycle and proof helpers."""
...
def prove_delegation_attenuation(
    *,
    ...
    uncovered = tuple(
        ...
        if not any(source_scope.covers(scope) for source_scope in source)
    )
    failures = [
    ...
    if (
        ...
        and parse_time(grant.expires_at) <= parse_time(evaluated_at)
    ):
    ...
    if allowed_policy_versions is not None and grant.policy_version not in set(
        allowed_policy_versions
    ):
        failures.append(
            f"delegation policy version {grant.policy_version!r} is not allowed"
        )
