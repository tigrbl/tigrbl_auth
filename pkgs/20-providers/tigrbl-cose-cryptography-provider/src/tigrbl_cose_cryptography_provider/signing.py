...
def sign_detached_signature(
    *, algorithm: int, private_key: object, signing_input: bytes
) -> bytes:
    descriptor = resolve_cose_algorithm(algorithm)
    key = (
        serialization.load_pem_private_key(bytes(private_key), password=None)
