...
async def rotate_rsa_jwt_key() -> str:
    ...
    for item in existing_keys:
        if isinstance(item, dict) and item.get("kid") and item not in _rotation_jwks_cache:
            _rotation_jwks_cache.append(item)
    if previous_kid and all(item.get("kid") != previous_kid for item in _rotation_jwks_cache):
        try:
...
__all__ = [
    "ensure_rsa_jwt_key", "id_token_service", "rsa_key_provider",
    "rotate_rsa_jwt_key", "rotation_jwks_cache",
]
