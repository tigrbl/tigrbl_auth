...
class SDKEcosystemCatalog:
    ...
    def compatible_sdk_ids(
        self, *, runtime_version: str, language: str | None = None
    ) -> tuple[str, ...]:
        compatible: list[str] = []
        ...
        *,
        ...
        for package in self._packages.values():
            if not version_in_range(runtime_version, package.compatible_runtime_range):
                mismatches[package.sdk_id] = {
                    "runtime": runtime_version,
                    "reason": "runtime version out of range",
                }
                continue
            contract_mismatch = {
        ...
        return {
            ...
            "compatible_sdk_ids": list(
                self.compatible_sdk_ids(runtime_version=runtime_version)
            ),
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "languages": languages,
            "release_channels": sorted(
                {package.release_channel for package in self._packages.values()}
            ),
        }
...
class PluginRuntimeRegistry:
    def __init__(self) -> None:
        ...
        self._hook_handlers: dict[
            str, dict[str, Callable[[Mapping[str, Any]], Mapping[str, Any]]]
        ] = {}
    ...
    def run_hook(
        self, plugin_id: str, hook_name: str, context: Mapping[str, Any]
    ) -> Mapping[str, Any]:
        ...
        try:
            ...
            Exception
        ) as exc:  # pragma: no cover - exact exceptions are plugin-defined.
            ...
            if (
                descriptor.fail_behavior == "disable_on_error"
                or "disable" in descriptor.operator_controls
            ):
                self.disable_plugin(plugin_id)
            raise RuntimeError(
                f"plugin hook failed in isolated execution: {message}"
            ) from exc
        self._events.append(
            PluginLifecycleEvent(
                event_id=f"plg-{uuid4().hex}",
    ...
    def summary(self) -> dict[str, Any]:
        enabled = [
            plugin.plugin_id for plugin in self._plugins.values() if plugin.enabled
        ]
        return {
            ...
            "isolation_modes": sorted(
                {plugin.isolation_mode for plugin in self._plugins.values()}
            ),
        }


from . import governance_reviews as _reviews  # noqa: E402
from .governance_reviews import *  # noqa: E402,F401,F403

__all__ = _reviews.__all__
