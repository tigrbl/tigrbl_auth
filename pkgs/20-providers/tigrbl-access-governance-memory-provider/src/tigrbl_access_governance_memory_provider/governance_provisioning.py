...
class ScimProvisioningPlane:
    ...
    def register_schema(
        self, schema_id: str, *, resource_kind: str, required_fields: Iterable[str]
    ) -> ScimSchema:
        schema = ScimSchema(
            schema_id=schema_id,
    ...
    def provision_user(
        ...
        *,
        ...
        schema = next(
            (item for item in self._schemas.values() if item.resource_kind == "User"),
            None,
        )
        if schema is None:
            raise RuntimeError(
                "user schema must be registered before provisioning users"
            )
        missing_fields = [
            field for field in schema.required_fields if field not in attributes
        ]
        if missing_fields:
            raise ValueError(
                f"user payload missing required fields: {', '.join(missing_fields)}"
            )
        user = ScimUser(
            user_id=user_id,
            tenant_id=tenant_id,
    ...
    def provision_group(
        ...
        *,
        ...
        schema = next(
            (item for item in self._schemas.values() if item.resource_kind == "Group"),
            None,
        )
        if schema is None:
            raise RuntimeError(
                "group schema must be registered before provisioning groups"
            )
        group = ScimGroup(
            group_id=group_id,
            tenant_id=tenant_id,
    ...
    def add_group_member(
        self, group_id: str, *, tenant_id: str, member_id: str
    ) -> ScimGroup:
        group = self._groups[group_id]
        if group.tenant_id != tenant_id:
            raise PermissionError("group tenant mismatch")
        updated = replace(
            group, members=tuple(sorted(set(group.members + (member_id,))))
        )
    ...
    def tenant_snapshot(self, tenant_id: str) -> dict[str, Any]:
        ...
        groups = [
            group for group in self._groups.values() if group.tenant_id == tenant_id
        ]
        return {
            "tenant_id": tenant_id,
