...
from .abac import ABACAdministrator
from .delegated_admin import DelegatedAdministrator
from .rbac import RBACAdministrator
...
class PolicyEngine:
    def __init__(
        ...
        *,
        ...
        if db is None and (rbac is None or abac is None or delegated_admin is None):
            raise ValueError(
                "db is required when PolicyEngine constructs storage-backed administrators"
            )
        self.db = db
    ...
    async def evaluate(
        ...
        *,
        ...
        if service_auth is not None:
            if service_auth.service.tenant_id != tenant_id:
                decision = PolicyDecision(
                    False,
                    "permission denied by service identity tenant mismatch",
                    (service_auth.service.name,),
                )
                self._record_audit(
                    decision,
                    subject=subject,
                ...
                return decision
            if not any(
                _permission_matches(grant, permission)
                for grant in service_auth.granted_permissions
            ):
                decision = PolicyDecision(
                    False,
                    "permission denied by service identity scopes",
                    (service_auth.service.name,),
                )
                self._record_audit(
                    decision,
                ...
                return decision
            rbac_decision = PolicyDecision(
                True,
                "permission granted by service identity scope",
                (service_auth.service.name,),
            )
        else:
        ...
        requires_abac = await self.abac.has_relevant_policy(
            permission, tenant_id, client_id
        )

        if rbac_decision.allowed and (abac_decision.allowed or not requires_abac):
            matched = rbac_decision.matched + (
                () if not requires_abac else abac_decision.matched
            )
            reason = (
                rbac_decision.reason
                if not requires_abac
                else "permission granted by RBAC assignment and ABAC attributes"
            )
    ...
    async def _build_compliance_report(
        ...
        *,
        ...
        tenant_ids_tuple = tuple(
            sorted(set(str(tenant_id) for tenant_id in tenant_ids))
        )
